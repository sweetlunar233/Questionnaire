...
import { forEach } from "core-js/core/array";
 ...
 export default({
  ...
  methods: {
    ...
    isCorrect(index) {
      if(this.questionListFill[index].radio == this.questionListDesign[this.questionListFill[index].questionId])
      return this.selectedOption === this.correctOption;
    },
    // 判断当前选项是否为正确选项
    isSelectedCorrectOption(index) {
      let answeroption = 0;
      (this.questionListDesign[this.questionListFill[index].questionId].optionList).forEach(option=>{
    ...
    addSingle(){
        this.questionCnt++;
        this.questionListFill.push({"type":1,"questionnumber":0,"isNecessary":true,"question":"请选择一个选项","radio":ref(''),
        "optionCnt":2,"optionList":[{"optionId":0,"content":"选项"},{"optionId":1,"content":"选项"}]});
    },
    //TieZhu:添加多选题
    addMultiple(){
        this.questionCnt++;
